...
from ray.rllib.agents.maddpg.maddpg_torch_model import build_maddpg_models, _make_continuous_space
...
def maddpg_actor_critic_loss(
    ...
    act_t = model.get_policy_output(model_out_t)
    policy_t = model.dist

    target_model_out_tp1, _ = policy.target_model(input_dict_next, [], None)

    act_tp1 = policy.target_model.get_policy_output(target_model_out_tp1)
    policy_tp1 = policy.target_model.dist
    ...
    # Q-values for given actions & observations in given current state
    q_t = model.get_q_values(model_out_n, act_n)

    # Q-values for current policy (no noise) in given current state
    policy_n = act_n
    policy_n[agent_id] = act_t
...
MADDPGTorchPolicy = build_policy_class(
    ...
    get_default_config=lambda: ray.rllib.agents.maddpg.maddpg.DEFAULT_CONFIG,
